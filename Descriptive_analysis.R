...
setwd("C:/Users/stepa/OneDrive/Dokumenty/Working_Paper_FSV_MFF")
...
sum(data$left_mentions)
2561/222253*100 #v kolika % projevů
#pravice
data$right_mentions <- str_count(data$text, regex(pattern_right))
sum(data$right_mentions)
x/222253*100
...
EF<-mutate(EF, ment_pps = left_mentions/speeches)
EF<-mutate(EF, perc = id/speeches*100)


ggplot(by_year, aes(x = month, y = x, color = party, group = party)) +
  geom_line(size = 1) +
  geom_point(size = 1) +
  theme_barbie()+
  geom_smooth(method="loess")+
